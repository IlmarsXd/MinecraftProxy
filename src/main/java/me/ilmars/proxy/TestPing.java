...
public class TestPing {
    ...
    private void ping(final String ip, final int port) {
        state = "Pinging " + ip + "...";
        NetworkManager networkmanager = createNetManager(ip, port);
        ...
        networkmanager.setNetHandler(new INetHandlerStatusClient() {
            ...
            public void handlePong(S01PacketPong packetIn) {
                ...
                if (proxyIp.isEmpty() || proxyIp.equals(ProxyHandler.lastProxyIp)) {
    ...
    private NetworkManager createNetManager(String ip, int port) {
        try {
            return NetworkManager.createNetworkManagerAndConnect(InetAddress.getByName(ip), port, false);
